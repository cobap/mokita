...
def processa_dados(df):
    ...
    with pd.ExcelWriter('database_reparos.xlsx') as writer:
        ...
        labor_df.to_excel(writer, sheet_name='labor', columns=['key', 'numero_sr', 'parque', 'subcategoria', 'data_inicio', 'hora_inicio', 'data_fim', 'hora_fim', 'descricao'], index=False)
        ...
        df.to_excel(writer, sheet_name='atividades', columns=['key', 'numero_sr', 'complexo', 'data_do_debrief', 'numerotecnicos', 'paccase', 'problemcode', 'resolutioncode', 'subcategoria', 'tiporepair'], index=False)
